...
// Flat
//var per_line_scores = [0, 2, 4, 6];
// Superadditive
var per_line_scores = [0, 1, 3, 6];
// Subadditive
//var per_line_scores = [0, 3, 6, 6];
...
function getAllScores(found_list)
{
    ...
    for(var i = 0; i < words_per_player; i++)
    {
        ...
        var score_line = per_line_scores[found_line];
...
function equalSplit(found_list)
{
    // Get total
    var total = getAllScores(found_list)['total'];
...
function proportionalSplit(found_list)
{
    // Get total
    var total = getAllScores(found_list)['total'];
...
// Shapley values
function shapleySplit(found_list)
{
    // Calculate scores
    var scores = getAllScores(found_list);
...
function adjustSplitNaive(split, diff)
{
    var ret = [];
    for(var i = 0; i < split.length; i++)
    {
...
function adjustSplit(split, player, pts)
{
    // Copy
    var ret = [];
    for(var i = 0; i < split.length; i++)
...
function checkEqual(split_1, split_2)
{
...
function generateRewardOptions(found_list)
{
    // Find base options
    // TODO: break this out into a separate function and select function based 
    //       on experiment condition
    var options = [
    {'id': 1, 'split': roundDown(proportionalSplit(found_list))},
    {'id': 2, 'split': roundDown(adjustSplit(proportionalSplit(found_list), 0, 1))},
    {'id': 3, 'split': roundDown(adjustSplit(proportionalSplit(found_list), 1, 1))},
    {'id': 4, 'split': roundDown(adjustSplit(proportionalSplit(found_list), 2, 1))},
    ];
    ...
    for(var i = 0; i < options.length; i++)
    {
        ...
        for(var j = 0; j < i; j++)
        {
            if(checkEqual(options[i]['split'], options[j]['split']))
            {
                remove[i] = true;
            }
        }
...
{    
    ...
    var equal = equalSplit(found_list);
    console.log(equal)
    var prop = proportionalSplit(found_list);
    console.log(prop)
    var shapley = shapleySplit(found_list);
    console.log(shapley)
    var adj = adjustSplit(equal, 0, 1);
    console.log(adj)
}

// testSplits(found_list)

var options = generateRewardOptions(found_list);
console.log(options);
