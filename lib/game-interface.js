...
function WordSearchInterface() 
{
    ...
    // current screen: loading, lobby, game, wait_score, or score
    this.mode = "loading";

    // ----- Lobby things
    // Timer for lobby countdown
    this.countdown_timer = new Timer();
    
    // Time limit for lobby countdown
    // TODO: get this from config?
    this.countdown_length = 5000;
    // DEBUG: make this really short
    this.countdown_length = 500;
...
WordSearchInterface.prototype.startLobby = function()
{
    // Switch to lobby
    this.mode = "lobby";
    this.render();
...
WordSearchInterface.prototype.startGame = function()
{
    ...
    // Start the timer and start the timer updater
    this.timer.startTimer();
    this.updateTimer();
}

WordSearchInterface.prototype.waitGameFinished = function()
{
    console.log("Waiting for players to finish");
    this.mode = "wait_score";
    this.render();
}

WordSearchInterface.prototype.startScoreScreen = function()
{
    console.log("Starting score screen");
    this.mode = "score";
...
WordSearchInterface.prototype.render = function()
{
    var functions = {
        ...
        "wait_score": this.renderWaitingScore,
...
WordSearchInterface.prototype.renderWaitingScore = function()
{
    var html_string = "";
    html_string += '<div id="lobby-container">';
    html_string += '<div class="lobby-title">Game Over!</div>';
    html_string += '<div class="score-options-header">Waiting for all players to finish...</div>';
...
WordSearchInterface.prototype.renderScoreOption = function(option_id, option_num, payments)
{
    ...
    var p1_string   = "<div class='score-option-p1' style='width:" + percentages[0] + "%'>" + payments[0] + "</div>";
    var p2_string   = "<div class='score-option-p2' style='width:" + percentages[1] + "%'>" + payments[1] + "</div>";
    var p3_string   = "<div class='score-option-p3' style='width:" + percentages[2] + "%'>" + payments[2] + "</div>";
    
    var html_string = "<div class='score-option-line'>" + 
...
WordSearchInterface.prototype.updateTimer = function()
{
    ...
    var time_left = this.game_length - time_elapsed;
    ...
    if(this.mode == "game")
    {
        if(time_left <= 0)
        {
            this.waitGameFinished()
        }
        else
        {
            var that = this;
            setTimeout(function(){that.updateTimer();}, 200);
        }
    }
}
